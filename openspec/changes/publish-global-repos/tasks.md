...
- [ ] 1.3 Review the staged `config-opencode` file list and diff before the first commit to confirm no sensitive or host-specific files slipped in.

## 2. Bootstrap the two local repos
- [ ] 2.1 Initialize git in `~/.config/opencode` and create the curated initial `config-opencode` commit.
- [ ] 2.2 Initialize git in `~/.local/share/openspec/schemas/house-style` and create the initial `house-style-sdd` commit from the installed schema files.

## 3. Create and push the private GitHub repos
- [ ] 3.1 Create `PedrocaOBanido/config-opencode` and `PedrocaOBanido/house-style-sdd` as private repos with `gh`.
- [ ] 3.2 Add `origin` remotes and push the initial commit from each local repo.
- [ ] 3.3 Verify both remotes have the expected private visibility and published contents.
